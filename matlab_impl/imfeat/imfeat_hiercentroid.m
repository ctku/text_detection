function [param] = imfeat_hiercentroid(cmd1, cmd2, param)

switch cmd1
    case 'initialization'
        param.rd.mode = 'gray';
    case 'extract_feature_raw'
        param.feat_raw = imfeat_hiercentroid_algo(param.image, cmd2);
    otherwise
...
function [vec] = imfeat_hiercentroid_algo(image, cmd2)

    if cmd2==1
        [vec levels] = hierarchicalCentroid(image, 7, 1);
    else
        [vec levels] = hierarchicalCentroid(image);
    end
