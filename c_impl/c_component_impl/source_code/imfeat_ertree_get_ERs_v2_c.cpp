...
// *******************************************************************
static int debug = 0;
static MSERGrowHistory* hist_start; //kevin added
...
static bool MSERStableCheck( MSERConnectedComp* comp, MSERParams params )
{
	...
	comp->var = var;
	comp->history->stable = var; // Kevin added
	//comp->dvar = dvar;
	if ( stable )
			 ...
			 int reverse)
{
	...
	MSERGrowHistory* history = (MSERGrowHistory*)malloc( src->rows*src->cols*sizeof(history[0]) );
	for (int i=0; i<(src->rows*src->cols); i++)
		history[i].val = -1;
	//MemStorage storage;

	// darker to brighter (MSER-)
	MSERParams params( 1, 0, 0, 0, 0, 0, 0, 0, 0 );
	CvMat* mask = 0;
	CvSeq* contours = 0;
	MSERConnectedComp comp[257];
	imgptr = preprocessMSER_8UC1( &img, heap_start, src, mask, reverse );
	extractMSER_8UC1_Pass( ioptr, imgptr, heap_start, pts, history, comp, step, stepmask, stepgap, params, -1, contours );
	...
	MSERGrowHistory* histptr = history;
	int no_ER=0;
	while (histptr->val != -1) {
...
int main(void) {
	...
	// artificail image
		...
		u8* img_ptr = img_data;
		img_ptr[0] = 1; img_ptr[1] = 2; img_ptr[2] = 3; img_ptr[3] = 4;
		img_ptr[4] = 2; img_ptr[5] = 2; img_ptr[6] = 3; img_ptr[7] = 4;
		img_ptr[8] = 3; img_ptr[9] = 3; img_ptr[10] = 3; img_ptr[11] = 4;
		img_ptr[12] = 4; img_ptr[13] = 4; img_ptr[14] = 4; img_ptr[15] = 4;
	#elif 1
		// src 2
		int img_cols = 4;
		int img_rows = 4;
