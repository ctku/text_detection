...
typedef struct MSERNodeHistory
{
	...
	struct MSERNodeHistory* to_parent;
	struct MSERNodeHistory* to_firstChild;
	struct MSERNodeHistory* to_nextSibling;
	struct MSERNodeHistory* to_prevSibling;
	int val;
	int size;
}
MSERNodeHistory;
...
MSERNewHistory( MSERConnectedComp* comp, MSERNodeHistory* history )
{
	...
	if ( NULL == comp->history )
	{
		...
		if ( -1 == history->ER_firstChild )
		{
			...
			/*
			// when new a hist, if comp has hist and cur hist has child, assin it ??
			history->to_firstChild->ER_nextSibling = comp->history->ER_id;
			history->to_firstChild->to_nextSibling = comp->history;
			comp->history->ER_prevSibling = history->to_firstChild->ER_id;
			comp->history->to_prevSibling = history->to_firstChild;
			*/
		}
		...
		MSERNodeHistory* cur = comp->history->to_nextSibling;
		while ( cur )
		{
		  ...
		  MSERNodeHistory* history )
{
	...
	if ( NULL != comp1->history )
	{
		if ( -1 == history->ER_firstChild )
		{
			...
			/*
			// when merge, if comp has hist and cur hist has child, assign ??
			history->to_firstChild->ER_nextSibling = comp1->history->ER_id;
			history->to_firstChild->to_nextSibling = comp1->history;
			comp1->history->ER_prevSibling = history->to_firstChild->ER_id;
			comp1->history->to_prevSibling = history->to_firstChild;
			*/
		}
	...
	if ( NULL != comp2->history )
	{
		// check if there exists sibling (in comp's hist) or cur hist
		if (comp2->history->ER_val == comp1->grey_level)
		{
			 ...
			 int reverse)
{
	...
	for (int i=0; i<(src->rows*src->cols); i++) {
		...
		history[i].ER_val = -1;
		history[i].ER_size = -1;
		history[i].ER_parent = -1;
		history[i].ER_firstChild = -1;
		history[i].ER_nextSibling = -1;
		history[i].ER_prevSibling = -1;
		history[i].to_parent = NULL;
		history[i].to_firstChild = NULL;
		history[i].to_nextSibling = NULL;
		history[i].to_prevSibling = NULL;
	}
	//MemStorage storage;
